    ...
    <div class="ui grid m-0">
        ...
        <script v-if="ovenPlayerLoaded">
            ...
            socket.addEventListener('message', async function(event) {
                ...
                let userInfo = await axios.get('https://api.voxtl.tv/users/' + data.data.user + '/profile', { headers: { 'Authorization': '{{this.$auth.getToken('local')}}' } });
...
<script>
    ...
    export default {
        ...
        async asyncData(context) {
            let userProfilePromise = axios.get(`https://api.voxtl.tv/users/${context.params.watch}/profile`, { headers: { 'Authorization': `${context.$auth.getToken('local')}` } });
            let userChannelPromise = axios.get(`https://api.voxtl.tv/users/${context.params.watch}/channel`, { headers: { 'Authorization': `${context.$auth.getToken('local')}` } });
        ...
        mounted() {
            this.intervalID = window.setInterval(() => {
                axios.get(`https://api.voxtl.tv/users/${this.streamer.user.id}/channel`, { headers: { 'Authorization': `${this.$auth.getToken('local')}` } }).then(res => {
